...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Shift service dependency to access the blockchain
	var shift *shf.Shift
	if err := ctx.Service(&shift); err != nil {
		return nil, err
	}
	// Construct the release service
	contract, err := NewReleaseOracle(config.Oracle, shf.NewContractBackend(shift))
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		case <-timer.C:
			...
			if version.Major > r.config.Major ||
				...
				howtofix := fmt.Sprintf("Please check https://github.com/shiftcurrency/shift/releases for new releases")
