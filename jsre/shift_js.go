...
module.exports = function (str, isNew) {
    if (str.substr(0, 2) === '0x' && !isNew) {
        ...
        console.warn('see https://github.com/shift-org/web3.js/pull/205');
...
var unitMap = {
    ...
    'pshift':       '1000000000000000000000000000000'
};
 ...
 * - kwei       femtoshift     ada
 * - mwei       picoshift      babbage
 * - gwei       nanoshift      shannon      nano
 * - --         microshift     szabo        micro
 * - --         millishift     finney       milli
 * - shift      --             --
 * - kshift                    einstein     grand
 * - mshift
 * - gshift
 * - tshift
 *
...
web3.shf.isSyncing = function (callback) {
    return new IsSyncing(callback);
};

/*jshint maxparams:4 */
web3.shf.filter = function (fil, callback) {
    return new Filter(fil, watches.shf(), formatters.outputLogFormatter, callback);
};
/*jshint maxparams:3 */
...
// ADD defaultblock
Object.defineProperty(web3.shf, 'defaultBlock', {
    get: function () {
...
Object.defineProperty(web3.shf, 'defaultAccount', {
    get: function () {
...
AllSolidityEvents.prototype.execute = function (options, callback) {
    ...
    return new Filter(o, watches.shf(), formatter, callback);
};
...
var checkForContractAddress = function(contract, abi, callback){
    ...
    // wait for receipt
    var filter = web3.shf.filter('latest', function(e){
        if(!e && !callbackFired) {
            count++;
            ...
            if(count > 50) {
                ...
                web3.shf.getTransactionReceipt(contract.transactionHash, function(e, receipt){
                    if(receipt && !callbackFired) {

                        web3.shf.getCode(receipt.contractAddress, function(e, code){
                            /*jshint maxcomplexity: 5 */
...
ContractFactory.prototype.new = function () {
    ...
    if(callback) {
        ...
        web3.shf.sendTransaction(options, function (err, hash) {
            if (err) {
                callback(err);
            } else {
        ...
        var hash = web3.shf.sendTransaction(options);
        // add the transaction hash
        contract.transactionHash = hash;
...
SolidityFunction.prototype.call = function () {
    ...
    if (!callback) {
        var output = web3.shf.call(payload, defaultBlock);
        return this.unpackOutput(output);
    }

    var self = this;
    web3.shf.call(payload, defaultBlock, function (error, output) {
...
SolidityFunction.prototype.sendTransaction = function () {
    ...
    if (!callback) {
        return web3.shf.sendTransaction(payload);
    }

    web3.shf.sendTransaction(payload, callback);
};
...
/// @returns an array of objects describing web3.shf api methods
...
var transferToAddress = function (from, to, value, callback) {
    return web3.shf.sendTransaction({
        address: to,
        from: from,
        value: value
    }, callback);
...
web3.shf.contract = require('./lib/web3/contract');
web3.shf.namereg = namereg.namereg;
web3.shf.ibanNamereg = namereg.ibanNamereg;
web3.shf.sendIBANTransaction = require('./lib/web3/transfer');
web3.shf.iban = require('./lib/web3/iban');
