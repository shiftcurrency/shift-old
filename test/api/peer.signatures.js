...
describe('POST /peer/signatures', function () {
	...
	var transaction = node.lisk.transaction.createTransaction('1S', 1, node.gAccount.password);
	...
	describe('sending transaction from multisignature account', function () {
		...
		// Send transaction
		before(function (done) {
			transaction = node.lisk.multisignature.createTransaction('1S', 1, owner.password);
