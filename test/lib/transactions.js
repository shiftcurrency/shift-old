...
// Account info for a RANDOM account (which we create later) - 0 SHIFT amount | Will act as delegate
...
before(function (done) {
    ...
    setTimeout(function () {
        randomSHIFT = node.randomSHIFT();
        expectedFee = node.expectedFee(randomSHIFT);
        node.api.put("/transactions")
            ...
            .end(function (err, res) {
                ...
                if (res.body.success == true && res.body.transactionId != null) {
                    // console.log("Sent to " + Account1.address + " " + (randomSHIFT / node.normalizer) + " SHIFT");
                    // console.log("Expected fee (paid by sender): " + expectedFee / node.normalizer + " SHIFT");
                    Account1.transactions.push(transactionCount);
                    transactionCount += 1;
                    totalTxFee += (expectedFee / node.normalizer);
                    Account1.balance += randomSHIFT;
                    transactionList[transactionCount - 1] = {
                        "sender": node.Gaccount.address,
                        "recipient": Account1.address,
                        "brutoSent": (randomSHIFT + expectedFee) / node.normalizer,
                        "fee": expectedFee / node.normalizer,
                        "nettoSent": randomSHIFT / node.normalizer,
                        "txId": res.body.transactionId,
                        "type":node.TxTypes.SEND
                    }
                } else {
                    // console.log("Sending SHIFT to Account1 failed.");
                    // console.log("Sent: secret: " + node.Gaccount.password + ", amount: " + randomSHIFT + ", recipientId: " + Account1.address );
                    node.expect("TEST").to.equal("FAILED");
                }
                done();
            });
...
before(function (done) {
    ...
    setTimeout(function () {
        ...
        node.api.put("/transactions")
            ...
            .end(function (err, res) {
                // console.log(JSON.stringify(res.body));
                // console.log("Sending SHIFT from genesis account to account. Recipient is: " + Account2.address);
                // console.log("Sent to " + Account2.address + " " + (randomSHIFT / node.normalizer) + " SHIFT");
                // console.log("Expected fee (paid by sender): " + expectedFee / node.normalizer + " SHIFT");
                node.expect(res.body).to.have.property("success").to.be.true;
                if (res.body.success == true && res.body.transactionId != null) {
                    ...
                    // console.log("Sending SHIFT to Account2 failed.");
                    // console.log("Sent: secret: " + node.Gaccount.password + ", amount: " + randomSHIFT + ", recipientId: " + Account2.address );
                    node.expect("TEST").to.equal("FAILED");
                }
                done();
            });
...
describe("GET /api/transactions", function () {
    ...
    it("Using completely invalid fields. Should fail", function (done) {
        var senderId = "notAReadAddress", blockId = "about5", recipientId = "SHIFTLIOnair3", limit = "aLOT", offset = "Boris", orderBy = "blockId:asc";
