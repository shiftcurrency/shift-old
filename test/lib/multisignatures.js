...
var NoSHIFTAccount = node.randomAccount();
NoSHIFTAccount.name = "noshift";
...
function openAccount (account, i, done) {
    node.api.post("/accounts/open")
        ...
        .end(function (err, res) {
            ...
            } else if (account.name == "noshift") {
                NoSHIFTAccount.address = res.body.account.address;
                NoSHIFTAccount.publicKey = res.body.account.publicKey;
...
// Used for sending SHIFT to accounts
...
function sendSHIFT (account, i, done) {
    ...
    node.api.put("/transactions")
        ...
        .end(function (err, res) {
            ...
            // console.log("Sending " + randomSHIFT + " SHIFT to " + account.address);
            node.expect(res.body).to.have.property("success").to.be.true;
...
function sendSHIFTfromMultisigAccount (amount, recipient, done) {
    node.api.put("/transactions")
        ...
        .end(function (err, res) {
            // console.log(JSON.stringify(res.body));
            // console.log("Sending " + amount + " SHIFT to " + recipient);
            node.expect(res.body).to.have.property("success").to.be.true;
...
before(function (done) {
    async.series([
        ...
        function (seriesCb) {
            return openAccount(NoSHIFTAccount, null, seriesCb);
        },
        function (seriesCb) {
        ...
        function (seriesCb) {
            return sendSHIFT(MultisigAccount, null, seriesCb);
        }
    ], function (err) {
...
describe("PUT /multisignatures", function () {
    ...
    it("When account has 0 SHIFT. Should fail", function (done) {
        node.api.put("/multisignatures")
            .set("Accept", "application/json")
            .send({
                secret: NoSHIFTAccount.password,
                lifetime: 1,
                min: requiredSignatures,
                keysgroup: Keys
            })
