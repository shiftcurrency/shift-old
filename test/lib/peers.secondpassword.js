                ...
                .end(function (err, res) {
                    ...
                    node.api.put("/transactions")
                        ...
                        .send({
                            ...
                            amount: node.Fees.secondPasswordFee + 100000000, // Testing 1 delegate registration + 1 transaction sending 1Shift
...
describe("POST /peer/transactions", function () {
    ...
    describe("Sending normal transaction with second passphrase now enabled", function () {
        ...
        it("Using blank second signature. Should fail", function (done) {
            var transaction = node.shift.transaction.createTransaction("1L", 1, account.password, ""); // Send 1 Shift to address 1L
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("nethash", node.config.nethash)
        ...
        it("Using fake second signature. Should fail", function (done) {
            var transaction = node.shift.transaction.createTransaction("1L", 1, account.password, account2.secondPassword); // Send 1 Shift to address 1L
        ...
        it("Using valid second signature. Should be ok", function (done) {
            var transaction = node.shift.transaction.createTransaction("1L", 1, account.password, account.secondPassword); // Send 1 Shift to address 1L
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("nethash", node.config.nethash)
