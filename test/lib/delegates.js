...
// Second RANDOM account - 0 SHIFT amount | Will test registration with same delegate name, changing case
...
describe("PUT /accounts/delegates with funds", function () {
    ...
    before(function (done) {
        // Send random SHIFT amount from genesis account to Random account
        ...
        node.api.put("/transactions")
            ...
            .end(function (err, res) {
                ...
                if (res.body.success && res.body.transactionId) {
                    ...
                    Raccount.amount += node.SHIFT;
                } else {
                    // console.log("Transaction failed or transactionId is null");
                    // console.log("Sent: secret: " + node.Gaccount.password + ", amount: " + node.SHIFT + ", recipientId: " + Raccount.address);
                    node.expect("TEST").to.equal("FAILED");
                }
                done();
            });
    ...
    before(function (done) {
        // Check that Raccount has the SHIFT we sent
            ...
            node.api.post("/accounts/open")
                ...
                .end(function (err, res) {
                    ...
                    if (res.body.success && res.body.account) {
                        node.expect(res.body.account.balance).to.be.equal(String(node.SHIFT));
                    } else {
...
describe("PUT /delegates with funds", function () {
    ...
    before(function (done) {
        // Send random SHIFT amount from foundation account to second Random account
            ...
            node.onNewBlock(function (err) {
                ...
                node.api.put('/transactions')
                    ...
                    .end(function (err, res) {
                        ...
                        if (res.body.success && res.body.transactionId) {
                            node.expect(res.body.transactionId).to.be.above(1);
                            R2account.amount += node.SHIFT;
                        } else {
                            // console.log("Transaction failed or transactionId is null");
                            // console.log("Sent: secret: " + node.Gaccount.password + ", amount: " + node.SHIFT + ", recipientId: " + R2account.address);
                            node.expect("TEST").to.equal("FAILED");
                        }
                        done();
                    });
    ...
    before(function (done) {
        // Check that R2account has the SHIFT we sent
        ...
        node.onNewBlock(function (err) {
            ...
            node.api.post('/accounts/open')
                ...
                .end(function (err, res) {
                    ...
                    node.expect(res.body).to.have.property("success").to.be.true;
                    if (res.body.success && res.body.account) {
                        node.expect(res.body.account.balance).to.be.equal(''+node.SHIFT);
                    } else {
                        // console.log("Failed to open account or account object is null");
...
describe("GET /accounts/delegates?address=", function () {
    ...
    it("Using invalid address. Should fail", function (done) {
        node.api.get("/accounts/delegates?address=NOTaShiftAddress")
            .set("Accept", "application/json")
            .expect("Content-Type", /json/)
            .expect(200)
            .end(function (err, res) {
