...
// Package xeth is the interface to all Shift functionality.
package xeth
...
func NewTest(shf *shf.Shift, frontend Frontend) *XEth {
	return &XEth{backend: shf, frontend: frontend}
}

// New creates an XEth that uses the given frontend.
// If a nil Frontend is provided, a default frontend which
// confirms all transactions will be used.
func New(shift *shf.Shift, frontend Frontend) *XEth {
	xeth := &XEth{
		backend:          shift,
		frontend:         frontend,
		quit:             make(chan struct{}),
		filterManager:    filters.NewFilterSystem(shift.EventMux()),
	...
	shift.Miner().Register(xeth.agent)
	if frontend == nil {
