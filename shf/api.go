...
func (s *PrivateDebugAPI) TraceTransaction(txHash common.Hash, logger *vm.LogConfig) (*ExecutionResult, error) {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(s.shf.ChainDb(), txHash)
	if tx == nil {
		return nil, fmt.Errorf("transaction %x not found", txHash)
	}
	block := s.shf.BlockChain().GetBlock(blockHash)
	if block == nil {
		return nil, fmt.Errorf("block %x not found", blockHash)
	}
	// Create the state database to mutate and eventually trace
	parent := s.shf.BlockChain().GetBlock(block.ParentHash())
	if parent == nil {
		return nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	stateDb, err := state.New(parent.Root(), s.shf.ChainDb())
	if err != nil {
		return nil, err
	}
	...
	for idx, tx := range block.Transactions() {
		...
		if uint64(idx) < txIndex {
			vmenv := core.NewEnv(stateDb, s.config, s.shf.BlockChain(), msg, block.Header(), vm.Config{})
		...
		vmenv := core.NewEnv(stateDb, s.config, s.shf.BlockChain(), msg, block.Header(), vm.Config{Debug: true, Logger: *logger})
