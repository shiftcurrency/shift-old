...
// Shift is the address that mining rewards will be send to
func (s *PublicShiftAPI) Shiftbase() (common.Address, error) {
	return s.e.Shiftbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Shiftbase)
...
func (s *PublicShiftAPI) ProtocolVersion() *rpc.HexNumber {
	return rpc.NewHexNumber(s.e.ShfVersion())
}
...
func NewPrivateAccountAPI(e *Shift) *PrivateAccountAPI {
...
// PublicBlockChainAPI provides an API to access the Shift blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.shf.BlockChain().Export(out); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.shf.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.shf.BlockChain().InsertChain(blocks); err != nil {
...
func NewPrivateDebugAPI(config *core.ChainConfig, shf *Shift) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, shf: shf}
}

// ChaindbProperty returns leveldb properties of the chain database.
func (api *PrivateDebugAPI) ChaindbProperty(property string) (string, error) {
	ldb, ok := api.shf.chainDb.(interface {
		LDB() *leveldb.DB
	})
	if !ok {
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.Config) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.shf.BlockChain().GetBlock(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(block *types.Block, config *vm.Config) (bool, []vm.StructLog, error) {
	...
	statedb, err := state.New(blockchain.GetBlock(block.ParentHash()).Root(), api.shf.ChainDb())
	if err != nil {
		return false, collector.traces, err
	}
...
func (api *PrivateDebugAPI) SetHead(number uint64) {
	api.shf.BlockChain().SetHead(number)
}
...
func (api *PrivateDebugAPI) TraceTransaction(txHash common.Hash, logger *vm.LogConfig) (*ExecutionResult, error) {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(api.shf.ChainDb(), txHash)
	if tx == nil {
		return nil, fmt.Errorf("transaction %x not found", txHash)
	}
	block := api.shf.BlockChain().GetBlock(blockHash)
	if block == nil {
		return nil, fmt.Errorf("block %x not found", blockHash)
	}
	// Create the state database to mutate and eventually trace
	parent := api.shf.BlockChain().GetBlock(block.ParentHash())
	if parent == nil {
		return nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	stateDb, err := state.New(parent.Root(), api.shf.ChainDb())
	if err != nil {
		return nil, err
	}
	...
	for idx, tx := range block.Transactions() {
		...
		if uint64(idx) < txIndex {
			vmenv := core.NewEnv(stateDb, api.config, api.shf.BlockChain(), msg, block.Header(), vm.Config{})
		...
		vmenv := core.NewEnv(stateDb, api.config, api.shf.BlockChain(), msg, block.Header(), vm.Config{Debug: true, Logger: *logger})
...
// Version returns the current shift protocol version.
