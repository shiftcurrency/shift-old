...
func newTester(t *testing.T, confOverride func(*shf.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Shift service within
	...
	shfConf := &shf.Config{
		...
		Shiftbasebase:      common.HexToAddress(testAddress),
		AccountManager: accman,
		PowTest:        true,
	}
	if confOverride != nil {
		confOverride(shfConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return shf.New(ctx, shfConf) }); err != nil {
