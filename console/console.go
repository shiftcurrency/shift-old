...
import (
	...
	"github.com/shiftcurrency/shift/internal/web3ext"
...
type Config struct {
	...
	Client   rpc.Client   // RPC client to execute Shift requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jshf", struct{}{})

	jshfObj, _ := c.jsre.Get("jshf")
	jshfObj.Object().Set("send", bridge.Send)
	jshfObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jshf);"); err != nil {
	...
	if c.prompter != nil {
		...
		// Assign the jshf.unlockAccount and jshf.newAccount in the Console the original web3 callbacks.
		// These will be called by the jshf.* methods after they got the password from the user and send
		// the original web3 request to the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jshf.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jshf.newAccount = personal.newAccount;`); err != nil {
...
// Welcome show summary of current Gshift instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gshift metadata
	fmt.Fprintf(c.printer, "Welcome to the Gshift JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + eth.coinbase);
		console.log("at block: " + eth.blockNumber + " (" + new Date(1000 * shf.getBlock(eth.blockNumber).timestamp) + ")");
