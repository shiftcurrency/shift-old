...
func NewAdminApi(xeth *xeth.XEth, shift *shf.Shift, codec codec.Codec) *adminApi {
	return &adminApi{
		xeth:     xeth,
		shift: shift,
		codec:    codec,
...
func (self *adminApi) AddPeer(req *shared.Request) (interface{}, error) {
	...
	err := self.shift.AddPeer(args.Url)
	if err == nil {
		return true, nil
	}
	return false, err
...
func (self *adminApi) Peers(req *shared.Request) (interface{}, error) {
	return self.shift.Network().PeersInfo(), nil
}

func (self *adminApi) NodeInfo(req *shared.Request) (interface{}, error) {
	return self.shift.Network().NodeInfo(), nil
}

func (self *adminApi) DataDir(req *shared.Request) (interface{}, error) {
	return self.shift.DataDir, nil
}
...
func (self *adminApi) ImportChain(req *shared.Request) (interface{}, error) {
	...
	for batch := 0; ; batch++ {
		...
		// Import the batch.
		if hasAllBlocks(self.shift.BlockChain(), blocks[:i]) {
			continue
		}
		if _, err := self.shift.BlockChain().InsertChain(blocks[:i]); err != nil {
...
func (self *adminApi) ExportChain(req *shared.Request) (interface{}, error) {
	...
	defer fh.Close()
	if err := self.shift.BlockChain().Export(fh); err != nil {
		return false, err
	}
...
func (self *adminApi) StartRPC(req *shared.Request) (interface{}, error) {
	...
	apis, err := ParseApiString(args.Apis, self.codec, self.xeth, self.shift)
	if err != nil {
		return false, err
	}
...
func (self *adminApi) GetContractInfo(req *shared.Request) (interface{}, error) {
	...
	infoDoc, err := natspec.FetchDocsForContract(args.Contract, self.xeth, self.shift.HTTPClient())
	if err != nil {
		return nil, err
	}
...
func (self *adminApi) HttpGet(req *shared.Request) (interface{}, error) {
	...
	resp, err := self.shift.HTTPClient().Get(args.Uri, args.Path)
	if err != nil {
		return nil, err
	}
