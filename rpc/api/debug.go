...
func NewDebugApi(xeth *xeth.XEth, shift *shf.Shift, coder codec.Codec) *debugApi {
	return &debugApi{
		xeth:     xeth,
		shift: shift,
...
func (self *debugApi) DumpBlock(req *shared.Request) (interface{}, error) {
	...
	stateDb, err := state.New(block.Root(), self.shift.ChainDb())
	if err != nil {
		return nil, err
	}
...
func (self *debugApi) ProcessBlock(req *shared.Request) (interface{}, error) {
	...
	var (
		blockchain = self.shift.BlockChain()
	...
	statedb, err := state.New(blockchain.GetBlock(block.ParentHash()).Root(), self.shift.ChainDb())
	if err != nil {
		return false, err
	}
