...
func runBlockTest(ctx *cli.Context) {
	...
	if testname == "" {
		...
		for name, test := range bt {
			...
			shift, err := runOneBlockTest(ctx, test)
			if err != nil {
				fmt.Println(err)
				fmt.Println("FAIL")
				ecode = 1
	...
	if rpc {
		...
		startEth(ctx, shift)
		utils.StartRPC(shift, ctx)
		shift.WaitForShutdown()
	}
}

func runOneBlockTest(ctx *cli.Context, test *tests.BlockTest) (*shf.Shift, error) {
	...
	shift.ResetWithGenesisBlock(test.Genesis)
	// import pre accounts
	_, err = test.InsertPreState(db, cfg.AccountManager)

	if err != nil {
		return shift, fmt.Errorf("InsertPreState: %v", err)
	}

	cm := shift.BlockChain()
	validBlocks, err := test.TryBlocksInsert(cm)
	if err != nil {
		return shift, fmt.Errorf("Block Test load error: %v", err)
	}
	newDB, err := cm.State()
	if err != nil {
		return shift, fmt.Errorf("Block Test get state error: %v", err)
	}
	if err := test.ValidatePostState(newDB); err != nil {
		return shift, fmt.Errorf("post state validation failed: %v", err)
	}
	return shift, test.ValidateImportedHeaders(cm, validBlocks)
