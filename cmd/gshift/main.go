...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	var shift *shf.Shift
	if err := stack.Service(&shift); err != nil {
		utils.Fatalf("shift service not running: %v", err)
	}
	accman := shift.AccountManager()
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) {
		if err := shift.StartMining(ctx.GlobalInt(utils.MinerThreadsFlag.Name), ctx.GlobalString(utils.MiningGPUFlag.Name)); err != nil {
