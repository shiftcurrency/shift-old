...
func init() {
	...
	app = utils.NewApp(Version, "the shift command line interface")
	app.Action = run
...
`,
		},
		{
			Action: version,
			Name:   "version",
			Usage:  "print shift version numbers",
			Description: `
...
will prompt for your password and imports your shift presale account.
...
func attach(ctx *cli.Context) {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to gshift node - %v", err)
	}
...
func startEth(ctx *cli.Context, shf *shf.Shift) {
	// Start Shift itself
	utils.StartShift(shf)

	am := shf.AccountManager()
	...
	if !ctx.GlobalBool(utils.IPCDisabledFlag.Name) {
		if err := utils.StartIPC(shf, ctx); err != nil {
			utils.Fatalf("Error string IPC: %v", err)
		}
	}
	if ctx.GlobalBool(utils.RPCEnabledFlag.Name) {
		if err := utils.StartRPC(shf, ctx); err != nil {
