...
var (
	...
	monitorCommand = cli.Command{
		Action: monitor,
		Name:   "monitor",
		Usage:  `Gshift Monitor: node metrics monitoring and visualization`,
		Description: `
The Gshift monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) {
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by gshift (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached gshift node and retrieves the entire set
