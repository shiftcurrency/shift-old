...
func TestAccounts(t *testing.T) {
	...
	checkEvalJSON(t, repl, `shf.accounts`, `["`+testAddress+`","`+addr+`"]`)
...
func TestBlockChain(t *testing.T) {
	...
	// do the export
	extmp, err := ioutil.TempDir("", "gshift-test-export")
	if err != nil {
		t.Fatal(err)
	}
	...
	checkEvalJSON(t, repl, `debug.dumpBlock(shf.blockNumber)`, beforeExport)
}
...
func TestCheckTestAccountBalance(t *testing.T) {
	...
	checkEvalJSON(t, repl, `shf.getBalance(primary)`, `"`+testBalance+`"`)
}
...
func TestContract(t *testing.T) {
	...
	* then set once more SetHashReg SetUrlHint
	...
	if checkEvalJSON(t, repl, `primary = shf.accounts[0]`, `"`+testAddress+`"`) != nil {
		return
	}
	...
	if err != nil {
		...
		if checkEvalJSON(t, repl, `contract = shf.compile.solidity(source).test`, string(contractInfo)) != nil {
			return
		}
	}
	...
	if checkEvalJSON(
		t, repl,
		`contractaddress = shf.sendTransaction({from: primary, data: contract.code})`,
