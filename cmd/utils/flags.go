...
// MakeEthConfig creates shift options from set command line flags.
func MakeEthConfig(clientID, version string, ctx *cli.Context) *shf.Config {
	...
	if ctx.GlobalBool(DevModeFlag.Name) {
		...
		if !ctx.GlobalIsSet(DataDirFlag.Name) {
			cfg.DataDir = os.TempDir() + "/shift_dev_mode"
		}
		cfg.PowTest = true
		cfg.DevMode = true
...
func MakeChain(ctx *cli.Context) (chain *core.BlockChain, chainDb ethdb.Database) {
	...
	chain, err = core.NewBlockChain(chainDb, nil, pow, eventMux)
	if err != nil {
...
func IpcSocketPath(ctx *cli.Context) (ipcpath string) {
	if runtime.GOOS == "windows" {
		...
		ipcpath = common.DefaultIpcPath()
		if ctx.GlobalIsSet(DataDirFlag.Name) {
			ipcpath = filepath.Join(ctx.GlobalString(DataDirFlag.Name), "gshift.ipc")
		}
		if ctx.GlobalIsSet(IPCPathFlag.Name) {
			ipcpath = ctx.GlobalString(IPCPathFlag.Name)
		}
...
func StartIPC(shf *shf.Shift, ctx *cli.Context) error {
	config := comms.IpcConfig{
		Endpoint: IpcSocketPath(ctx),
	}


	initializer := func(conn net.Conn) (comms.Stopper, shared.ShiftApi, error) {
		fe := useragent.NewRemoteFrontend(conn, shf.AccountManager())
		xeth := xeth.New(shf, fe)
		apis, err := api.ParseApiString(ctx.GlobalString(IPCApiFlag.Name), codec.JSON, xeth, shf)
		if err != nil {
			return nil, nil, err
		}
...
func StartRPC(shf *shf.Shift, ctx *cli.Context) error {
	...
	xeth := xeth.New(shf, nil)
	codec := codec.JSON

	apis, err := api.ParseApiString(ctx.GlobalString(RpcApiFlag.Name), codec, xeth, shf)
	if err != nil {
		return err
	}
